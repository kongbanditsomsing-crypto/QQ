    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for t in raw_tokens:
            ...
            if is_valid:
                ...
                token_details.append(f"{t_type}: {name}")
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if val == "1":
            ...
            if not user_data or not user_data.get("tokens"):
                ...
                desc = f"<a:1000030093:1551252638794780883> กำลังทำงาน tokenทั้งหมดที่กำลังดักมีทั้งหมด ({len(tokens)}) เเละนี่คือโปรไฟล์tokenเเละประเภท:\n"
                for idx, t in enumerate(tokens, 1):
                    desc += f"{idx}. ประเภท: **{t['type']}** | ชื่อ: **{t['name']}**\n"
...
async def on_message(message: discord.Message):
    ...
    # Scan Attachments for QR Code if no link found in text
